...
class DAO {
	...
	private $GETPOLOZENO = "SELECT * FROM rezultat WHERE brIndexa=?";

    private $PROSECNA = "SELECT AVG(ocena) AS avg_ocena FROM rezultat";
    ...
    public function getPolozeno($brIndexa){
        $statement = $this->db->prepare($this->GETPOLOZENO);
    ...
    {
        $statement = $this->db->prepare($this->PROSECNA);
        $statement->execute();
        $result = $statement->fetch();
        return $result['avg_ocena'];
    }
